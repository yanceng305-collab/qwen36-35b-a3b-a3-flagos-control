...
Diagnostic A已定位prospective U+FFFD首先出现在exact tokenizer native decode并在downstream chain保持一致。User已批准D-034 provenance-aware branch：native decode与全部downstream text/codepoints一致时记录`TOKENIZER_NATIVE_UFFFD`且不单独触发corruption-attribution FAIL；任何post-tokenizer mutation立即FAIL/STOP；其他functional/output-quality gates继续执行。原 formal rerun 已在 F0 因 device-scope mapping drift STOP；D-035 local STOP尚未同步入Control，D-036 supplemental mapping证明该次run的composite placement，remaining blocker为provenance correlation。Stage 6仍STOP/NOT ACCEPTED，唯一下一步是[`QWEN36-A3-S6-PROVENANCE-CORRELATION-AND-FUNCTIONAL-MATRIX-RERUN`](tasks/QWEN36-A3-S6-PROVENANCE-CORRELATION-AND-FUNCTIONAL-MATRIX-RERUN.md)，状态READY并等待明确User dispatch。Stage 6、performance、prefix和EP2继续locked。
