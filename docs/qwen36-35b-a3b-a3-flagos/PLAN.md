...
状态：Stage 1/2、Stage 3、Stage 4和Stage 5均已 **ACCEPTED**；Stage 6 `QWEN36-A3-S6-A2-EQUIVALENT-FUNCTIONAL-MATRIX`已 **STOP / NOT ACCEPTED / FORMALLY REVIEWED**。Evidence-first `QWEN36-A3-S6-UFFFD-OUTPUT-CHAIN-DIAGNOSTIC`已执行，结论 **D / UNRESOLVED**，Codex1 Acceptance **PENDING**。
...
| 6D — U+FFFD output-chain diagnostic | existing Evidence优先，定位raw response/token/decode/save/validator chain | [Task](tasks/QWEN36-A3-S6-UFFFD-OUTPUT-CHAIN-DIAGNOSTIC.md)；explicit User dispatch received | A/B/C evidence-backed classification，或D unresolved；不产生Stage 6 PASS | **DIAGNOSTIC STOP — D / UNRESOLVED；Codex1 Acceptance PENDING** |
